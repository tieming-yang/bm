...
export default function ThreeDSandbox() {
  return (
    <div className="relative min-h-svh min-w-svw">
      <UnlockedModelOverlay modelURL={TEST_MODEL_URL} />
      <ModelTitleOverlay title={TEST_TITLE} zhTitle={TEST_ZH_TITLE} />
    </div>
  );
}
...
function ModelTitleOverlay({ title, zhTitle }: { title: string; zhTitle: string }) {
    ...
    <div className="pointer-events-none absolute inset-x-0 top-6 z-20 flex justify-center px-4">
      <div className="pointer-events-auto flex max-w-md flex-col items-center text-center">
        <button
          ...
          className="relative min-h-16 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary focus-visible:ring-offset-2 focus-visible:ring-offset-background md:min-h-24"
          style={{ perspective: 1200 }}
          onClick={() => setShowZhTitle((current) => !current)}
        >
          <AnimatePresence initial={false} mode="wait">
            <motion.span
              key={showZhTitle ? "zh-title" : "en-title"}
            ...
            >
              {showZhTitle ? zhTitle : toTitle(title)}
            </motion.span>
          </AnimatePresence>
        </button>
      </div>
