...
export default function BibleBooks({}: Props) {
  ...
  return (
    <motion.section
      ...
      className="h-svh"
    >
      <ul className="z-20 px-3 grid grid-cols-1 md:grid-cols-2 w-svw md:px-5 xl:px-7 place-content-center gap-2">
        {Object.entries(BibleArtworks.groupedByBook(currentLanguage)).map(([book, collections]) => (
          <li key={collections[0].id} className="flex flex-col gap-y-3">
            ...
            <h2 className="font-serif text-3xl font-semibold text-primary">{t(book)}</h2>
          </li>
        ))}
      </ul>
    </motion.section>
